...
for i in range(1,len(fields)+1):
    plt.subplot(3,3,i)
    df_copy[fields[i-1]].value_counts().sort_values().plot.bar(color='#DEB078')  
...
plt.boxplot([df_copy['capital-loss']])
plt.xticks([])
...
plt.title('Boxplot for capital_gain')
plt.boxplot([df_copy['capital-gain']])
plt.xticks([])
...
p = df_copy.select_dtypes(include='number').columns.to_list()
p.append("income")
y=3
...
"""
# Lets check skewness of numerical columns. It seems that capital-gain and capital-loss are heavily skewed.
df_copy['age'].skew()
df_copy['fnlwgt'].skew()
df_copy['education-num'].skew()
df_copy['capital-gain'].skew()
df_copy['capital-loss'].skew()
df_copy['hours-per-week'].skew()


# Skewed data can be problematic for some ML algorithms. Lets log transform 'capital-gain' and 'capital-loss'
df_copy[['capital-gain', 'capital-loss']] = df_copy[['capital-gain', 'capital-loss']].apply(lambda x: np.log(x+1))
...
# Significant predictors and their interpretations (for random state 1000)
#
...
# We will treat this as our baseline model.
...
y_pred2 = np.where(lr.predict_proba(X_test)[:,1] > best_threshold_maximizing_F1, 1, 0) # Getting predicted classes of the testing data at our tuned threshold value
...
print("Accuracy =", Accuracy)
print("Precision =", Precision)
print('F1 score of Testing Set =', f1_score(y_test, y_pred2))
...
# RandomForest, 5-fold CV with hyperparameter tuning (n_estimators, max_features, and min_samples_leaf)
...
test_rf = RandomizedSearchCV(estimator=rf, param_distributions=start_grid, cv=5)
print(start_grid)
...
test_grid = GridSearchCV(estimator = rf, param_grid=gs_grid, cv=5)
res = test_grid.fit(X_train_valid, y_train_valid)
print(res.best_params_)
print(res.best_score_)

final_model =  RandomForestClassifier(n_estimators = res.best_params_['n_estimators'], 
                                      max_features = res.best_params_['max_features'],
                                      min_samples_leaf = res.best_params_['min_samples_leaf'], random_state = 1000)
final_model.fit(X_train_valid, y_train_valid)

predictions = res.predict(X_test)
print(accuracy_score(y_test, predictions))

print('Random Forest - Best Hyperparameters:', res.best_params_)
print('Random Forest - F1 Score on Testing Set:', f1_score(y_test, predictions))
print('Random Forest - Confusion Matrix for Testing Set:\n', confusion_matrix(y_test, predictions, labels = [1,0]))
print('Random Forest - Accuracy of Testing Set:', accuracy_score(y_test, predictions))

importance2 = pd.DataFrame({'feature':X.columns.values, 'importance':final_model.feature_importances_})
print('Random Forest - Features ranked according to their importance are as below: \n')
print(importance2[importance2['importance'] != 0].sort_values(by = ['importance'], ascending = False))


"""
{'n_estimators': 800, 'min_samples_leaf': 3, 'max_features': 6}

Random Forest - Best Hyperparameters: {'max_features': 4, 'min_samples_leaf': 2, 'n_estimators': 150}
Random Forest - F1 Score on Testing Set: 0.6558197747183979
Random Forest - Confusion Matrix for Testing Set:
 [[1310  972]
 [ 403 6350]]
Random Forest - Accuracy of Testing Set: 0.84781405644715

Random Forest - Features ranked according to their importance are as below: 

                         feature    importance
2                   capital-gain  1.642101e-01
14        marital-status_Married  1.165597e-01
0                            age  1.048211e-01
28          relationship_Husband  8.377738e-02
4                 hours-per-week  7.063949e-02
1                         fnlwgt  5.811150e-02
3                   capital-loss  4.413270e-02
17    occupation_Exec-managerial  3.949874e-02
9            education_Bachelors  3.523353e-02
23     occupation_Prof-specialty  3.340289e-02
29    relationship_Not-in-family  2.958305e-02
37                      sex_Male  2.476508e-02
32             relationship_Wife  2.380781e-02
31        relationship_Own-child  2.143021e-02
12             education_Masters  2.016381e-02
11    education_High-school-grad  1.624369e-02
13         education_Prof-school  1.596000e-02
21      occupation_Other-service  1.259648e-02
8     education_Associate-degree  1.143916e-02
5              workclass_Private  8.654131e-03
10           education_Doctorate  7.955342e-03
18    occupation_Farming-fishing  6.610083e-03
25              occupation_Sales  5.839414e-03
6        workclass_Self-employed  5.695798e-03
38             native-country_US  4.981299e-03
19  occupation_Handlers-cleaners  4.632283e-03
26       occupation_Tech-support  4.364129e-03
20  occupation_Machine-op-inspct  4.128422e-03
36                    race_White  3.862891e-03
15       occupation_Adm-clerical  3.769095e-03
27   occupation_Transport-moving  3.496081e-03
35                    race_Black  3.360954e-03
24    occupation_Protective-serv  1.981225e-03
30   relationship_Other-relative  1.976861e-03
34       race_Asian-Pac-Islander  1.574790e-03
33       race_Amer-Indian-Eskimo  5.956958e-04
22    occupation_Priv-house-serv  1.154516e-04
7          workclass_Without-pay  2.933607e-05
16       occupation_Armed-Forces  2.100494e-07
"""
...
data = [
    ("Log Regression", Accuracy, f1_score(y_test, y_pred2)),
    ("Decision Tree", accuracy_score(y_test, y_pred), f1_score(y_test, y_pred)),
    ("Random Forest", accuracy_score(y_test, predictions), f1_score(y_test, predictions)),
    ("Gradient Boosting", accuracy_score(y_test, y_hat), f1_score(y_test, y_hat))
]
